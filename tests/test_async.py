...
class TestAsyncValidation:
    ...
    async def test_async_validator(self):
        ...
        user = await User.create_async(username="newuser", email="user@example.com")
        assert user.username == "newuser"

        # Test validation failure
        with pytest.raises(ValueError, match="Username 'admin' already exists"):
            await User.create_async(username="admin", email="admin@example.com")
    ...
    async def test_async_builder(self):
        ...
        user = await (
            User.builder().username("testuser").email("test@example.com").build_async()
        )
        ...
        with pytest.raises(ValueError, match="Invalid email format"):
            await (
                User.builder().username("testuser").email("invalid-email").build_async()
            )
    ...
    async def test_combined_sync_async_validation(self):
        ...
        with pytest.raises(ValueError, match="Password too short"):
            await User.create_async(username="user", password="short")

        # Test async validation failure
        with pytest.raises(ValueError, match="Username not available"):
            await User.create_async(username="taken", password="SecurePass123")
